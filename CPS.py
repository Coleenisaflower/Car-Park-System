...
class Body(tk.Frame):
    ...
    def save(self):
        if self.no_of_spots_tw.get() == 0 or self.no_of_spots_fw.get() == 0:
            ...
            self.no_fw.config(text=str(self.no_of_spots_fw.get()))
            self.no_tw.config(text=str(self.no_of_spots_tw.get()))
    ...
    def treeviewTable(self):
        ...
        c = connect.cursor()
        c.execute(
            "SELECT ticket_no, ticket_type, plate_no, color, vehicle_type, datetime_issued, checkout, total_fee FROM TICKET INNER JOIN VEHICLE on VEHICLE.v_plate_no = TICKET.plate_no LEFT JOIN PAYS on PAYS.p_ticket_no = TICKET.ticket_no ORDER BY ticket_no DESC")
        table = c.fetchall()
    ...
    def search(self):
        ...
        c = connect.cursor()
        c.execute(
            "SELECT ticket_no, ticket_type, plate_no, color, vehicle_type, datetime_issued, checkout, total_fee FROM TICKET INNER JOIN VEHICLE on VEHICLE.v_plate_no = TICKET.plate_no LEFT JOIN PAYS on PAYS.p_ticket_no = TICKET.ticket_no WHERE plate_no LIKE (?) OR ticket_no LIKE (?) ORDER BY ticket_no DESC",
            ('%' + key + '%', '%' + key + '%'))
        table = c.fetchall()
        # Commit our query:
        connect.commit()
    ...
    def timeOut(self):
        ...
        if val[1] == 'FOUR':
            ...
            fees = c.fetchone()
            self.totalFee(timestamp, val[0], val[1], duration, fees[0], fees[1])
        elif val[1] == 'TWO':
    ...
    def totalFee(self, timestamp, ticket_no, plate_no, duration, base_fee, added_fee):
        ...
        connect = sqlite3.connect('cps.db')
        c = connect.cursor()
        c.execute('INSERT INTO PAYS (p_ticket_no, p_plate_no, checkout, total_fee) VALUES (?, ?, ?, ?)',
                  (ticket_no, plate_no, timestamp, total_fee))
